...
public:
    ...
    double first_order_abs_change(double delta_t) {
        return Ag * (ka_/(ka_-ke_)) * (e_exp(delta_t)-a_exp(delta_t));
    }

    double collapsed_abs_change(double delta_t) {
        return Ag * ke_ * e_exp(delta_t) * delta_t;
    }
    ...
    std::vector<DisplayPoint> propagate_distribution(const std::vector<TimelinePoint>& time_data) {
        ...
        for (size_t i = 1; i < time_data.size(); ++i) {
            ...
            const double x = std::abs((ka_-ke_)*delta_t);
...
int main() {
    ...
    std::cout << std::setprecision(3) << "Using half-life=" << t12 << " (h), ka="
        ...
        << dose << " mg every " << tau << " h." << std::endl;

    double end_t = RegimenBuilder::end_time_by_hl_of_doses(t12+20, n_doses);
